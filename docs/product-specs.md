...
- **Rendu retenu** (`app/ui/circuit-track.tsx`, Server Component) : **glow rouge** (accent), **animation de tracé au chargement** (`stroke-dashoffset`, keyframe `bx-draw`, respecte `prefers-reduced-motion`), **damier départ/arrivée** au 1ᵉʳ point, **flèche de sens** de parcours. Bandeau de stats sous le tracé : **longueur** (donnée bacinger), **nombre de tours** et **nombre de virages** (mappings statiques `lib/f1/circuit-static-data.ts`, ~24 circuits, fallback si absent). Écartés à ce stade : 1er GP / altitude, dégradé de trait, numéros de virages.
  ...
  - **Fiabilité tours/virages (décidé 2026-07-02, suivi #174)** : les valeurs statiques actuelles sont un point de départ (sans impact scoring, fallback si absentes). Plan validé pour une PR de suivi dédiée : **tours dérivés automatiquement des résultats F1** (le nombre de tours du vainqueur, déjà ingéré par le cron `/api/f1/sync`, lu sur la dernière édition disputée du circuit → auto-MAJ chaque saison), **virages conservés en table curée datée** (aucune API fiable ; scraping écarté car fragile pour une valeur qui ne change qu'à une reconfiguration physique) avec revue annuelle documentée.
