...
- [ ] **Durcissement sécurité DB (advisor Supabase)** — à traiter **avant ouverture publique**. Warnings remontés sur les fonctions des migrations fondation (PR #7 a déjà corrigé `create_league`) :
  ...
  - [ ] **Leaked password protection** désactivée côté Supabase Auth → l'activer (vérification HaveIBeenPwned) — action manuelle dashboard, en attente.
  - [ ] Vérifier via `get_advisors(security)` que la liste est vide après coup. État 08/07/2026 : findings `anon` éliminés ; restent 3 WARN **assumés** (`is_member_of_league`/`shared_league` exécutables par `authenticated` — indispensable aux policies RLS — et `delete_own_account` — feature suppression de compte) + le point leaked password ci-dessus.
