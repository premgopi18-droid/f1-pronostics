...
- [x] **Transparence des items + différenciation visuelle dans les résultats** ([issue #151](https://github.com/premgopi18-droid/f1-pronostics/issues/151), implémenté) — la page GP de ligue (`/leagues/[id]/gp/[gp-id]`, [`app/leagues/[id]/gp/[gpId]/page.tsx`](../app/leagues/[id]/gp/[gpId]/page.tsx)) n'explique pas le rôle des items dans les scores et manque de différenciation visuelle. Maquette de référence : [`docs/mockups/gp-results-items.html`](mockups/gp-results-items.html). Décisions :
  ...
  - **Réalisation** : libellés d'items centralisés dans [`lib/items/catalog.ts`](../lib/items/catalog.ts) (réutilisés par la page « Jouer un item ») ; narratif « Faits marquants » dans [`lib/items/facts.ts`](../lib/items/facts.ts) et détail positions/items dans [`lib/scoring/gp-detail.ts`](../lib/scoring/gp-detail.ts) (fonctions pures) ; tokens `silver`/`bronze` ajoutés au thème ; canal « moi » = token `accent` du thème. **Écart assumé sur le lazy-load** : le détail n'est pas chargé au clic mais calculé côté serveur pour tous les membres — les pastilles `●N`/`◐N` du classement exigent de toute façon les pronos de **tous** les membres (le compte des approchées n'est pas stocké), donc un fetch par clic serait redondant. Volume identique à `/compare` (≤20 membres × ≤4 sessions).
