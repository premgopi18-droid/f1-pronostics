...
> **Meilleur tour en course — publication Jolpica partielle** (décision 06/09/2026, incident Monza 2026 — #239) : Jolpica publie parfois le classement de la course **avant** le bloc `FastestLap` (à Monza, toujours absent 6 h après l'arrivée). Une course terminée a forcément un meilleur tour : un résultat sans aucun `fastest_lap` est une **publication incomplète, pas une information**. Règle : (1) si le classement Jolpica ne porte aucun meilleur tour, la sync le demande à **OpenF1** (`/laps`, meilleur `lap_duration` de la course — Jolpica reste primaire car il ne compte que les tours valides) ; (2) si aucune source ne l'a, la **confirmation de la course est différée** (même mécanisme que #212, mais **fenêtre dédiée de 6 h après le départ** — décision review PR #240 : différer la course retient toutes les positions pour un bonus de 7, et les pannes OpenF1 connues sont courtes ; le cron repasse toutes les 10 min le week-end, soit ~24 tentatives) — au-delà, on confirme sans meilleur tour avec une erreur visible dans les logs et un rattrapage manuel. Le fallback OpenF1 a été validé sur les 12 courses 2026 précédentes (12/12 concordances avec Jolpica, aucun tour aberrant) : pas de garde-fou de vraisemblance. Pas de rattrapage automatique après finalisation : Wild Card (moitié du score) et Double points (×2) dépendent du score de base, bonus inclus — les deltas d'items déjà résolus seraient faussés.
...
#### Tracé du circuit sur la page de pronostic (décidé 2026-07-02, #153)

En haut de `/predictions/[gpId]`, entre le header et les onglets, un **tracé SVG du circuit** contextualise les pronostics. Purement visuel, non interactif, sans librairie externe (projection TypeScript maison).

- **Source** : dépôt public `bacinger/f1-circuits` (GeoJSON, ~40 circuits F1). La *Feature* complète (géométrie + `properties`) est stockée telle quelle dans la table Supabase `circuit_tracks` (JSONB, lecture publique RLS), clé = id bacinger (ex. `mc-1929`).
- **Sync** : `POST /api/admin/sync-circuits` (protégé `CRON_SECRET` via `isCronAuthorized`) — fetch le fichier maître bacinger → upsert de tous les circuits. Appel manuel au déploiement / si un tracé change (pas de cron Vercel). Alternative de seed : script one-off réutilisant `createServiceClient`.
- **Mapping** : `lib/f1/circuit-mapping.ts` associe le nom Jolpica (`grands_prix.circuit`, ex. « Autodromo Nazionale di Monza ») à l'id bacinger. Noms vérifiés contre la prod (saison 2026). Circuit non mappé ou absent en base → **aucun tracé affiché (fallback gracieux)**.
- **Projection** (`lib/f1/circuit-svg.ts`) : normalisation lat/lng vers le viewBox avec **correction de distorsion longitudinale `cos(latitudeMoyenne)`** (1° de longitude ≠ 1° de latitude hors équateur), **ratio d'aspect préservé** (jamais d'écrasement horizontal), tracé centré. Tests Vitest `circuit-svg.test.ts`.
- **Rendu retenu** (`app/ui/circuit-track.tsx`, Server Component présentational) : **glow rouge** (accent), **animation de tracé au chargement** (`stroke-dashoffset`, keyframe `bx-draw`, respecte `prefers-reduced-motion`), **damier départ/arrivée** au 1ᵉʳ point, **flèche de sens** de parcours. Bandeau de stats sous le tracé : **longueur** (donnée bacinger), **nombre de tours** et **nombre de virages** (fallback si absent). Le composant reçoit `laps`/`turns` en props (résolus par la page). Écartés : 1er GP / altitude, dégradé de trait, numéros de virages.
  - **Repère damier/flèche** : dérivés de l'ordre brut des coordonnées bacinger (1ᵉʳ point + tangente) — pas garantis de refléter la vraie ligne de départ/arrivée ni le sens de course. Choix assumé pour un repère visuel ; override par circuit possible plus tard si un tracé s'affiche « à l'envers ».
  - **Tours (auto, #174)** : plus de mapping saisi à la main. Le nombre de tours de la course est dérivé des résultats F1 — le cron `/api/f1/sync` capture les tours du vainqueur à la confirmation de la course et les stocke dans `grands_prix.race_laps` (via `fetchRaceLaps` + `setRaceLaps`). La page lit la valeur de la **dernière édition disputée du circuit** (lookup par `grands_prix.circuit`, `race_laps` non nul, plus récent) → toujours à jour, se remplit tout seul après la 1ʳᵉ course d'un nouveau circuit (ex. Madring). Les éditions passées ont été backfillées.
  - **Virages (curés, #174)** : aucune source exploitable (ni bacinger, ni Jolpica, ni OpenF1) et scraping Wikipédia écarté (fragile, valeur qui ne bouge qu'à une reconfiguration physique). Table curée `TURNS_BY_CIRCUIT` (`lib/f1/circuit-static-data.ts`, clé = id bacinger). **Checklist de revue annuelle** (début de saison) : (1) revérifier les circuits reconfigurés ; (2) ajouter les nouveaux tracés du calendrier ; (3) mettre à jour la ligne « Dernière vérification » du fichier. Dernière vérification : **2026**.
- **Contrainte cross-browser** : jamais de `var(--x)` dans un attribut SVG `fill`/`stroke` (non résolu par Safari) — on utilise `currentColor` + classe de thème (`text-foreground` / `text-primary`).
...
- **Pilote forfait pour un GP** : il **reste sélectionnable** (la liste est par saison, pas par GP — aucune source fiable ne donne les engagés avant le week-end). Le pronostiquer rapporte 0 pt sur cette ligne (règle « pilote absent du classement »). Atténuation : le pré-remplissage du prono course utilise la grille de départ réelle, qui ne contient que les vrais partants.
  - **Signal « absent du week-end ? »** (décidé 20/08/2026, livré 21/08/2026 — issue #211) : la liste de pronos reflète les **engagés réels du GP** dès qu'une session **fiable** du week-end a été observée (`gp_lineups.observed_at` — une session est fiable **H+2 h 30 après son début** (durée max + latence de bascule OpenF1, cf. #214), **H+4 pour la course** (drapeaux rouges — décision 21\08\2026, #218) ; ne pas resserrer sous 2 h 30 sans calibration sur plusieurs GP). Trois effets, affichage seulement — **jamais de filtre dur ni de blocage à la validation** :
    - l'**écurie affichée** d'un pilote est celle du line-up observé du week-end (mapping explicite libellés OpenF1 → écuries internes), prioritaire sur la « dernière course disputée » — un pilote échangé apparaît sous sa vraie écurie du week-end AVANT la course ;
    - un pilote de la saison **jamais observé** alors que d'autres l'ont été porte un badge « Absent du week-end ? » et est relégué en fin de liste. Le « ? » est voulu : un titulaire qui saute les EL1 (run rookie) est badgé à tort quelques heures, auto-corrigé à la session fiable suivante ;
    - avant la première observation fiable (jeudi, vendredi matin) : liste saison classique, aucun signal.
    - Non retenu : créer les pilotes depuis OpenF1 quand Jolpica ne les liste pas encore — Jolpica s'est montré proactif (Tsunoda ajouté avant même les EL1), et le filet « confirmation différée » (#212) couvre le cas du retard résiduel.
  - **Liste saison > nombre de partants** (décidé 21\08\2026 — premier cas réel : 23 pilotes saison à Zandvoort après l'ajout de Lawson) : le formulaire **course** classe **exactement `POSITIONS_TO_SCORE.race` (22) pilotes**, jamais « toute la liste ». Quand la liste dépasse ce nombre :
    - les pilotes en surplus (badgés absents en priorité, relégués en fin de liste) démarrent dans une section **« Non classés »** sous la liste classée, avec une ligne d'explication (« probablement pas au départ — 0 point s'il ne court pas ») ;
    - chaque ligne classée gagne un bouton retirer (×) et la section non classés un bouton Ajouter (désactivé à 22\22) → échange possible, **jamais de blocage dur** (le signal absence reste une déduction) ;
    - sous-titre et compteur adaptés (« Classe les 22 partants », `n/22`), enregistrement possible uniquement à 22 exactement ;
    - si un pilote badgé « Absent du week-end ? » est classé quand même (course, qualifs ou sprint) : **avertissement inline** rappelant la conséquence (0 point s'il ne court pas) — choix éclairé, pas d'interdiction ;
    - à 22 pilotes pile (cas nominal), le formulaire course reste l'ordre complet actuel, sans section ni compteur.
...
- **Échange de baquet en cours de saison** (décision 19/08/2026) : deux cas distincts — **temporaire** (remplacement d'un GP, ex. réserviste qui dépanne puis rend le volant) et **définitif** (transfert). Les deux sont couverts par le même mécanisme : **l'écurie est attribuée par course**, pas par saison. Les résultats de course Jolpica incluent l'écurie de chaque pilote pour cette course → stockée dans `session_results` au sync. Conséquences :
  - **Item « It must be the water ! »** (`no_points_team`) : résolu contre le **duo réel de la course** (écuries des résultats), plus contre le mapping saison — un joueur qui vise Red Bull le week-end où Lawson remplace Hadjar est évalué sur Verstappen + Lawson, pas sur le line-up nominal.
  - **Label « écurie actuelle »** d'un pilote (affichage) : dérivé de sa **dernière course disputée** (fallback : standings Jolpica). La règle précédente « dernière écurie de la liste standings » restait fausse après un remplacement temporaire (l'écurie d'origine, déjà présente dans la liste, ne repasse jamais en dernière position au retour du pilote).
  - Pendant le week-end de l'échange, le label affiché peut rester celui d'avant (les données ne bougent qu'une fois la course courue) — assumé, purement cosmétique, aucun impact points.
  - **Vues rétrospectives** (résultats d'un GP passé, récap podium) : affichent l'écurie du pilote **ce jour-là** (`session_results`), pas son écurie actuelle — un GP d'il y a 6 mois reste historiquement juste après un transfert.
  - **Validation** : le code écurie d'un item `no_points_team` est validé côté serveur contre la table `constructors` de la saison (même règle que les pronos WCC) ; une écurie introuvable à la résolution ⇒ item **sans effet** (jamais de bonus par défaut).
- **Notification « changement de line-up »** (décision 19/08/2026) : quand un pilote roule pour une écurie différente de celle de sa dernière apparition (remplacement, échange de baquet), une **notif push est envoyée à tous les joueurs** avant la course. Détection automatique pendant le sync via OpenF1 (liste des pilotes de la première session du week-end disponible — EL1 dès le vendredi), en comparant au line-up du GP précédent (même source ⇒ pas de problème de normalisation des noms d'écurie). Un seul push agrégé par GP (« Lawson pilote pour Red Bull ce week-end »), dédup par claim atomique, pas de préférence dédiée (événement rare et à fort enjeu — même logique que les notifs deadline). En attendant la feature : annonce manuelle possible via `/api/admin/announce`.
...
- **Transition de vue entre écrans** : un `<ViewTransition enter/exit="bx-page" default="none">` autour du contenu de page (`app/components/page-transition.tsx`), monté par **`app/template.tsx`** — ré-instancié à chaque navigation, donc l'ancienne page **sort** et la nouvelle **entre** (jamais « update »). Conséquences voulues : une mutation en transition à l'intérieur d'une page (switch optimiste, `router.refresh`, révélation skeleton → contenu) n'anime **pas** l'écran ; l'ancienne page sort à sa propre géométrie (pas de morph de boîte, pas de désalignement si elle était scrollée). Animation sobre : sortie en fondu rapide (150 ms), entrée en fondu + léger glissement vers le haut (210 ms, décalée après la sortie). La bottom nav reste ancrée (hors de l'enveloppe). Le flag `experimental.viewTransition` est déclaré mais sans effet runtime en Next 16.2.9 (React canary vendu). Typings via `types/react-canary.d.ts`. Support : animation dessinée là où `view-transition-class` existe (Chrome 125+, Safari 18.2+) ; Safari 18.0–18.1 = fondu UA par défaut ; sans View Transitions API = cut sec.
- **Coupé** par `prefers-reduced-motion` et `.reduce-motion` : durée et délai à 0 sur les `::view-transition-*` (pseudo-éléments sur `<html>`, hors de portée du `*` ; `fill-mode: both` garde l'état final → swap net).
- **Mutations optimistes** quand l'état est binaire et réversible : bascule des inscriptions admin (`useOptimistic`, retour automatique à l'état confirmé si l'action échoue), préférences de notifications (déjà en place). Les valeurs qui viennent du serveur (ex. code d'invitation régénéré) sont **relues depuis les props** : une action qui `revalidatePath` embarque le re-render de la page dans sa réponse, un état local divergerait. Après un item joué, l'écran de confirmation reste affiché pendant que la page se rafraîchit en arrière-plan (plus de texte « Mise à jour de la page… »).
...
- **Mesure** : Vercel Speed Insights (tier gratuit, Real Experience Score par route) pour comparer avant / après. **Biais du splash (décision 2026-09-22, #251)** : les premières données montrent un LCP de 3,7 s sur la Home = durée du Lottie (3,2 s) + fondu (0,5 s), le navigateur ne comptant le contenu comme affiché qu'une fois l'overlay retiré — contre 488 ms sans splash. L'animation est **conservée telle quelle** ; les chargements où le splash a joué sont **exclus de la télémétrie** (`beforeSend`, marqueur global posé par le boot script pour ce chargement, `lib/splash/splash.ts`). Le score mesure donc ce sur quoi on peut agir, pas le premier chargement réel avec splash — à garder en tête en le lisant.
...
- **Fréquence** : joué **une seule fois par session** d'onglet (`sessionStorage`, posé par le boot script) — pas rejoué lors des navigations client. Le boot script pose aussi un marqueur global propre au chargement (`SPLASH_PLAYED_GLOBAL`) : les Web Vitals de ce chargement sont exclus de Speed Insights (cf. « Fluidité perçue », #251).
...
- [ ] **Durcissement sécurité DB (advisor Supabase)** — à traiter **avant ouverture publique**. Warnings remontés sur les fonctions des migrations fondation (PR #7 a déjà corrigé `create_league`) :
  - [x] `search_path` mutable sur `enforce_min_one_admin`, `enforce_max_members`, `handle_new_user` → épinglé sur les trois (vérifié en prod via `pg_proc.proconfig`, 08/07/2026).
  - [x] Fonctions `SECURITY DEFINER` exécutables par `anon`/`authenticated` via `/rest/v1/rpc/...` → `REVOKE EXECUTE` appliqué (migration `20260708130000_lock_security_definer_functions`, PR #186) : `handle_new_user` n'est plus appelable par personne via l'API (fonction trigger), `is_member_of_league`/`shared_league` sont réservées à `authenticated` (requis par les policies RLS). Rester en `SECURITY DEFINER` est volontaire (helpers anti-récursion RLS, trigger qui écrit dans `profiles`).
  - [ ] **Leaked password protection** (vérification HaveIBeenPwned) — **bloquée par le plan** : tentative d'activation le 08/07/2026 refusée par Supabase (« available on Pro Plans and up »), le projet est sur le plan Free. **Risque nul aujourd'hui** : l'app est en Google OAuth uniquement, aucun mot de passe n'est stocké. À activer si (1) passage au plan Pro, ou (2) ouverture d'un signup email/mot de passe — dans ce cas ce point redevient bloquant avant lancement.
  - [ ] Vérifier via `get_advisors(security)` que la liste est vide après coup. État 08/07/2026 : findings `anon` éliminés ; restent 3 WARN **assumés** (`is_member_of_league`/`shared_league` exécutables par `authenticated` — indispensable aux policies RLS — et `delete_own_account` — feature suppression de compte) + le WARN leaked password ci-dessus (plan-gated, sans objet en OAuth-only).
