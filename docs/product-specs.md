...
- **Bottom nav** : l'onglet visé s'allume dès le tap (`useLinkStatus` → état pending remonté à la nav, helper pur `isHighlightedTab` dans `lib/nav.ts`), l'icône pulse jusqu'à l'arrivée de la page. `aria-current` reste porté par le chemin réel.
