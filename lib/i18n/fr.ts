/**
 * Catalogue de traduction — français (locale de lancement).
 *
 * i18n approche A : 1 seule locale pour l'instant, pas de routing multi-locale.
 * Ajouter une langue plus tard = ajouter un catalogue de même forme, sans toucher
 * aux écrans (les appels `t('…')` restent identiques).
 */
export const fr = {
  // Libellés de sessions — jeu unique (#221), consommé via lib/i18n/session-labels.ts.
  session: {
    long: {
      qualifying: "Qualifications",
      race: "Course",
      sprint_qualifying: "Sprint Qualifs",
      sprint_race: "Sprint",
    },
    short: {
      qualifying: "Qualifs",
      race: "Course",
      sprint_qualifying: "Sprint Qualifs",
      sprint_race: "Sprint",
    },
  },
  common: {
    loading: "Chargement…",
    next: "Suivant →",
    back: "Précédent",
    close: "Fermer",
    comingSoon: "Bientôt disponible.",
  },
  login: {
    tagline1: "Pronostics F1 entre amis.",
    tagline2: "Ligues privées · items stratégiques.",
    cta: "Continuer avec Google",
    footer: "Pas d'installation · partage par lien",
  },
  onboarding: {
    pseudoStep: "Étape 1 / 2",
    pseudoTitle: "Choisis ton pseudo",
    pseudoSubtitle: "C'est le nom que verront tes amis dans les classements.",
    pseudoPlaceholder: "ex: BoxBoxRomain",
    pseudoChecking: "Vérification…",
    pseudoAvailable: "Disponible",
    avatarStep: "Étape 2 / 2",
    avatarTitle: "Choisis ta couleur",
    avatarSubtitle: "Elle t'identifie dans les classements. Tu peux aussi ajouter une photo (facultatif).",
    finish: "C'est parti ! 🏁",
  },
  home: {
    greeting: "Salut,",
    nextGpLabel: "Prochain GP",
    previousGpLabel: "Dernier GP",
    rawScore: "Ton score brut",
    points: "pts",
    round: "Manche",
    countdownDays: "jours",
    countdownHours: "heures",
    countdownMins: "min",
    countdownAriaLabel: "Départ dans {days} jours {hours} heures {mins} minutes",
    predict: "Je pronostique",
    noNextGp: "Aucun Grand Prix à venir pour le moment.",
    liveLabel: "LIVE",
    processing: "Calcul des scores — résolution des items…",
    weekendInProgress: "Week-end en cours",
    modifyPredictions: "Modifier mes pronos",
    viewResults: "Voir les résultats",
    sessionLocked: "Verrouillé",
    sessionOpen: "Ouvert",
    createLeague: "Créer une ligue",
    joinLeague: "Rejoindre 🔗",
    myLeagues: "Mes ligues",
    noLeaguesTitle: "Pas encore de ligue",
    noLeaguesText: "Crée ou rejoins une ligue pour commencer.",
    seasonLink: "Pronostics saison WDC / WCC",
    signOut: "Se déconnecter",
  },
  join: {
    invitationLabel: "Invitation reçue",
    adminLabel: "Admin",
    seasonLabel: "Saison",
    members: "membres",
    seasonInProgress: "en cours",
    blurb:
      "Pronostique chaque Grand Prix, joue des items contre tes amis, grimpe au classement de la saison.",
    cta: "Rejoindre la ligue",
    fullTitle: "Ligue complète",
    fullText: "Cette ligue a atteint son nombre maximum de membres.",
    closedTitle: "Inscriptions fermées",
    closedText: "L'admin a fermé les inscriptions à cette ligue.",
    notFoundTitle: "Invitation invalide",
    notFoundText: "Ce lien d'invitation est introuvable ou a expiré.",
    alreadyMemberTitle: "Tu es déjà dans cette ligue",
    alreadyMemberText: "Tu fais déjà partie de cette ligue pour cette saison.",
    alreadyMemberCta: "Voir la ligue",
    manualTitle: "Rejoindre une ligue",
    manualLabel: "Code d'invitation",
  },
  nav: {
    label: "Navigation principale",
    home: "Accueil",
    leagues: "Ligues",
    predictions: "Mes Pronos",
    results: "GP Résultats",
    profile: "Profil",
  },
  predict: {
    back: "Retour",
    absentFromWeekend: "Absent du week-end ?",
    noSessions: "Les sessions ne sont pas encore disponibles.",
    noPrediction: "Aucun pronostic soumis",
    emptyClassees: "Ajoute des pilotes ci-dessous",
    saved: "Sauvegardé ✓",
    open: "Ouvert",
    locked: "Verrouillé",
    courseSubtitle: "Ordre complet de la course · glisse ou tape pour sélectionner",
    courseSubtitleA11y: "Ordre complet · tape pour sélectionner ou utilise ↑↓",
    courseSubtitleCapped: "Classe les {count} partants · glisse ou tape pour sélectionner",
    courseSubtitleCappedA11y: "Classe les {count} partants · tape ou utilise ↑↓",
    absentHint: "« Absent du week-end ? » : probablement pas au départ — 0 point s'il ne court pas.",
    rankedAbsentWarning: "⚠️ {names} : marqué « Absent du week-end ? » — 0 point sans départ.",
    qualifsSubtitlePre: "Mon classement · top",
    qualifsSubtitlePost: "· glisse ou ajoute en tapant",
    qualifsSubtitleA11yPost: "· tape ou ↑↓",
    ranked: "CLASSÉS",
    unranked: "NON CLASSÉS",
    add: "Ajouter",
    reorder: "Réordonner",
    remove: "Retirer",
    moveUp: "Monter",
    moveDown: "Descendre",
    close: "Fermer",
    a11yHint: "Difficultés avec le drag & drop ?",
    a11yHintCta: "Activer le mode accessibilité",
    sessionsLabel: "Sessions",
    fastestLap: "Meilleur tour",
    fastestLapMeta: "1 pilote · bonus +7 pts",
    choosePilot: "Choisir un pilote",
    none: "— Aucun",
    save: "Enregistrer",
    saving: "Enregistrement…",
    savedOk: "Pronostic enregistré !",
    savedDraft: "Brouillon enregistré ({count}/{total}) — complète ton prono pour qu'il rapporte des points.",
    gridPrefilled: "Pré-rempli avec la grille de départ — enregistre pour valider ton prono.",
    gridPrefilledQualifying: "Pré-rempli avec l'ordre des qualifications — enregistre pour valider ton prono.",
    gridReset: "Repartir de la grille de départ",
    gridResetQualifying: "Repartir de l'ordre des qualifications",
    gridResetDone: "Ordre réinitialisé sur la grille de départ.",
    gridResetDoneQualifying: "Ordre réinitialisé sur l'ordre des qualifications.",
    circuit: {
      alt: "Tracé du circuit — {name}",
      lengthLabel: "Longueur",
      lengthUnit: "km",
      lapsLabel: "Tours",
      turnsLabel: "Virages",
    },
  },
  recap: {
    back: "Mes pronos",
    subtitleDefinitif: "Score définitif",
    subtitleProvisoire: "Score provisoire",
    badgeDefinitif: "Définitif",
    badgeProvisoire: "Provisoire",
    podiumTitle: "Podium officiel",
    scoreTitle: "Score de prédiction",
    exactes: "exactes",
    plusMinusUn: "à ±1",
    meilleurTour: "meilleur tour",
    pts: "pts",
    totalBrut: "Total brut",
    dansMesLigues: "Dans mes ligues",
    voirResultats: "Voir les résultats F1 officiels →",
    noScore: "Aucun score disponible pour ce GP.",
    noPrediction: "Aucun pronostic soumis",
    rankUp: "↑",
    rankDown: "↓",
    rankPasses: "tu passes",
    rankOf: "de la ligue",
  },
  compare: {
    back: "Scores du GP",
    title: "Pronos",
    visibleAfterLock: "Visible après verrouillage",
    modeLabel: "Mode de comparaison",
    vueGroupe: "Vue groupe",
    teteATete: "Tête-à-tête",
    officiel: "officiel",
    toi: "TOI",
    toiVs: "Toi vs",
    noSessionsLocked: "Aucune session verrouillée pour l'instant.",
    noOtherMembers: "Pas d'autres membres dans cette ligue.",
    predictionNone: "—",
    prevOpponent: "Adversaire précédent",
    nextOpponent: "Adversaire suivant",
  },
  // Catalogue canonique des items (nom + description longue). Source unique réutilisée
  // par la page « Jouer un item » et les « Faits marquants » des résultats GP. Les emojis
  // sont des tokens visuels (lib/items/catalog.ts), pas du texte traduisible.
  items: {
    playedOk:       "Item joué ! Il sera résolu après la course.",
    chooseOpponent: "Choisir un adversaire",
    chooseSession:  "Choisir une session",
    shield:         { name: "Bouclier",                     description: "Annule tous les items offensifs reçus ce week-end" },
    block_driver:   { name: "Bloquer un pilote",            description: "Un pilote rapporte 0 pt de position à un adversaire pour une session choisie" },
    wild_card:      { name: "Wild Card",                    description: "Vole la moitié des points d'un adversaire sur une session" },
    double_points:  { name: "Dernier tour de magie",        description: "×2 sur tes points de course ou de qualifications" },
    dnf_prediction: { name: "On va trancher dans le vif !", description: "Choisir un pilote qui ne finira pas la course — +8 pts s'il abandonne" },
    underdog_top5:  { name: "Il est de retour !",           description: "Un pilote qualifié hors top 10 finit dans le top 5 de la course — +8 pts" },
    no_points_team: { name: "\"It must be the water !\"",    description: "Aucun des 2 pilotes d'une écurie ne marque de point en course — +12 pts" },
    fia_penalty:    { name: "Move de la FIA",               description: "Bientôt disponible — source de données en cours d'intégration" },
    // Motifs de grisage d'un item indisponible (product-specs §3.5 — états d'indisponibilité).
    unavailable: {
      weekly_slot_taken: "Déjà joué ce week-end",
      exhausted:         "Épuisé pour la saison",
      phase_locked:      "Verrouillé — deadline passée",
    },
  },
  gpResults: {
    badgeDefinitif:  "Définitif",
    badgeProvisoire: "Provisoire",
    playItem:          "🎮 Jouer un item",
    itemsWeekend:      "🎮 Items du week-end",
    compareLink:       "👀 Pronos comparés",
    compareLockedHint: "Dispo après le départ de la 1ʳᵉ séance",
    noScores:        "Aucun score pour ce GP. Les scores apparaissent dès qu'une session est confirmée.",
    totalGp:         "Total GP",
    factsTitle:      "⚡ Faits marquants du GP",
    factsEmptyCalm:  "🐔 Pas un seul item joué ce week-end… vous attendiez une invitation ?",
    factsPending:    "🤐 Les coups fourrés seront révélés après la course.",
    detailTitle:     "Détail du joueur",
    detailHint:      "👆 Touche une ligne du classement pour voir un autre joueur",
    me:              "(moi)",
    totalGpSub:      "total GP",
    pts:             "pts",
    meilleurTour:    "Meilleur tour",
    round:           "Round",
    flNotPlayed:     "Non joué",
    // États vides
    emptyNoPredictionSession: "Pas de pronostic pour cette session.",
    emptyInvalid:             "Pronostic invalide",
    emptyNotSubmitted:        "Aucun pronostic soumis",
    // Légende du classement
    legendExact:   "exacte",
    legendApprox:  "approchée",
    legendGapHint: "détail : ±N = écart de positions",
    legendMe:      "ma ligne",
    // a11y — glyphes du détail/classement
    a11yExact:        "position exacte",
    a11yMiss:         "position ratée",
    a11yGapPrefix:    "écart de",
    a11yPositions:    "positions",
    a11yExactCount:   "positions exactes",
    a11yApproxCount:  "positions approchées",
    // Faits marquants — fragments
    factPlays:     "joue",
    factBlocks:    "bloque",
    factOn:        "sur",
    factVs:        "contre",
    factCancelled: "Annulé",
    // Faits marquants — cheminement (placeholders {actor} {target} {driver} {session} {pts})
    chainWildSteal:      "{actor} vole la moitié des points {session} de {target}",
    chainWildTransfer:   "−{pts} pour {target}, +{pts} pour {actor}",
    chainBlockShielded:  "🛡️ {target} avait posé un Bouclier",
    chainBlockCancelled: "→ blocage annulé, {driver} rapporte normalement (0)",
    chainWildCancelled:  "→ vol annulé, aucun point volé (0)",
    chainBlockRemoved:   "{driver} retiré du score {session} de {target} : −{pts}",
    chainBlockNoPoints:  "{driver} ne rapportait aucun point — sans effet",
    chainDoubleApplied:  "×2 sur le score {session} de {actor} : +{pts}",
    chainDoubleNoEffect: "Aucun score à doubler — sans effet",
    chainBonusApplied:   "Condition validée : +{pts}",
    chainBonusNoEffect:  "Condition non remplie — sans effet",
    chainNoPointsTeamApplied:  "{constructor} ne marque aucun point : +{pts}",
    chainNoPointsTeamNoEffect: "{constructor} a marqué des points — sans effet",
    chainShieldBlocked:  "Bouclier actif — a neutralisé {pts} attaque·s ce week-end",
    chainShieldUnused:   "Bouclier actif — aucune attaque reçue",
    // Lignes d'items dans le détail par joueur (placeholders {item} {target} {actor} {pts})
    detailItemOwnOffensive:   "{item} sur {target}",
    detailItemIncoming:       "{item} joué par {actor}",
    detailItemOwn:            "{item}",
    detailCancelledSuffix:    " · annulé",
    detailShieldNeutralized:  " · a neutralisé {pts} attaque·s",
  },
  leagues: {
    title: "Mes ligues",
    adminBadge: "ADMIN",
    members: "membres",
    seasonPoints: "pts saison",
    emptyText: "Crée une nouvelle ligue ou rejoins celle d'un ami.",
    create: "+ Créer",
    join: "Rejoindre une ligue",
    // Libellés pour lecteurs d'écran (aria-label)
    rankAriaLabel: "rang",
    rankAriaSeparator: "sur",
    remaining: "restant",
    remainingPlural: "restants",
    itemsAvailable: "Items disponibles",
  },
  leagueDetail: {
    season: "Saison",
    deadlineLabel: "Deadline pronos & items",
    leaderboardTitle: "CLASSEMENT SAISON",
    liveLabel: "En direct",
    myItemsTitle: "MES ITEMS DISPONIBLES",
    playItem: "Jouer un item",
    seasonPreviewTitle: "APERÇU SAISON",
    pts: "pts",
    allGps: "Voir tous les GPs →",
    lockedPredictions: "Voir les pronos verrouillés des membres →",
    weekendsTitle: "Week-ends",
    gpStatusFinal: "Définitif",
    gpStatusProvisional: "Provisoire",
    gpStatusNext: "Prochain",
    gpStatusUpcoming: "À venir",
    readinessTitle: "QUI EST PRÊT ?",
    readinessSprintWeekend: "Week-end sprint",
    readinessInProgress: "En cours",
    readinessCaption: "Pronos remplis par membre et par session — le contenu reste caché jusqu'au verrouillage",
    readinessStateSubmitted: "Envoyé",
    readinessStateMissing: "Manquant",
    readinessStateMissed: "Trop tard",
  },
  admin: {
    pageTitle: "Paramètres admin",
    inviteSection: "INVITER",
    shortCode: "Code court",
    fullLink: "Lien complet",
    copyCode: "Copier",
    copyLink: "Copier le lien",
    regen: "Régénérer",
    regenWarning: "L'ancien lien ne fonctionnera plus. Les membres actuels ne sont pas affectés.",
    cancel: "Annuler",
    confirm: "Confirmer",
    inscriptionsLabel: "Inscriptions",
    inscriptionsOpen: "Ouvertes",
    inscriptionsClosed: "Fermées",
    membersSection: "MEMBRES · TRANSFERT D'ADMIN",
    promoteAdmin: "Nommer admin",
    promoteWarning: "Tu perdras tes droits admin. Cette action est irréversible.",
    disclaimer: "L'admin ne peut pas exclure un membre. Suppression de ligue indisponible en cours de saison.",
  },
  createLeague: {
    title: "Créer une ligue",
    nameLabel: "Nom de la ligue",
    namePlaceholder: "ex: Les Pole Sitters",
    membersLabel: "Nombre maximum de membres",
    hardCap: "Hard cap · 20",
    infoTextPre: "Tu seras automatiquement",
    infoAdmin: "admin",
    infoTextPost: ". Le lien d'invitation s'affiche juste après la création.",
    submit: "Créer la ligue",
  },
  results: {
    seasonTitle: "Saison",
    tabCalendar: "Calendrier",
    tabDrivers: "Pilotes",
    tabTeams: "Écuries",
    badgeProchain: "PROCHAIN",
    badgeEnCours: "EN COURS",
    qualiLabel: "Quali",
    raceLabel: "Course",
    winner: "Vainqueur",
    linkResults: "Résultats",
    linkPredict: "Pronostiquer",
    upcoming: "À venir",
    noCalendar: "Aucun Grand Prix disponible.",
    // Détail GP
    gpBack: "Résultats",
    gpResultsOfficial: "Résultats officiels",
    tabRace: "Course",
    tabQualifying: "Qualifications",
    tabSprint: "Sprint",
    tabSprintQualifying: "Quali sprint",
    practiceGroupLabel: "Essais libres",
    tabPractice1: "EL 1",
    tabPractice2: "EL 2",
    tabPractice3: "EL 3",
    leader: "LEADER",
    dnfLabel: "DNF",
    dnsLabel: "DNS",
    fastestLap: "Meilleur tour",
    noResults: "Résultats non disponibles.",
    practiceNoData: "Résultats non encore disponibles.",
  },
  season: {
    savedOk:           "Pronostic saison enregistré !",
    moveHandle:        "Déplacer",
    moveUp:            "Monter",
    moveDown:          "Descendre",
    moveFromTitle:     "Depuis la position",
    moveToTitle:       "Vers la position",
    pageTitle:         "WDC / WCC — Pronostics saison",
    wdcTitle:          "Champion pilotes · WDC",
    wccTitle:          "Champion constructeurs · WCC",
    tablistLabel:      "Champion pilotes ou écuries",
    tabWdc:            "Pilotes · WDC",
    tabWcc:            "Écuries · WCC",
    lockedBadge:       "Verrouillé · comparaison live",
    deadlinePrefix:    "Deadline :",
    lockedSince:       "Pronostics verrouillés depuis le 1er GP",
    colMyPrediction:   "MON PRONO",
    colOfficial:       "OFFICIEL ACTUEL",
    colGap:            "ÉCART",
    statsExacts:       "Exacts",
    statsProjected:    "Score projeté",
    noPrediction:      "Aucun pronostic soumis pour ce classement.",
    itemWdcName:       "Coup de clé à molette",
    itemWdcDesc:       "Déplace 1 pilote dans ton prono",
    itemWccName:       "Boost turbo",
    itemWccDesc:       "Déplace 1 écurie dans ton prono",
    itemFromLabel:     "Depuis la position",
    itemToLabel:       "Vers la position",
    itemUsedSuffix:    "utilisé !",
    itemPanelSubtitle: "Déplace une entrée dans ton classement",
    use:               "Utiliser",
    confirm:           "Confirmer",
    sending:           "Envoi…",
    cancel:            "Annuler",
    back:              "Accueil",
    // standings dans app/results
    standingsNone:     "Classement non disponible.",
    standingsPts:      "pts",
  },
  myPronos: {
    // Section saison
    seasonTitle:      "Pronostics saison",
    wdcTitle:         "Champion pilotes · WDC",
    wccTitle:         "Champion constructeurs · WCC",
    submitted:        "Soumis",
    notSubmitted:     "Non soumis",
    itemsTitle:       "Items saison",
    itemAvailable:    "Disponible",
    itemUsed:         "Utilisé",
    // Section GP courant
    gpTitle:          "GP en cours / prochain",
    sessionSubmitted: "Soumis",
    sessionMissed:    "Verrouillé",
    sessionOpen:      "Ouvert",
    noCurrentGp:      "Aucun Grand Prix à venir pour le moment.",
    // Section historique
    historyTitle:     "Historique",
    rawScore:         "Score brut",
    noHistory:        "Aucun historique pour l'instant — ton premier GP arrive bientôt",
  },
  avatar: {
    helmetAlt: "Avatar casque",
    photoAlt: "Photo de profil",
    pickLegend: "Choisis ton casque",
    photo: {
      add: "Ajouter une photo",
      change: "Changer la photo",
      remove: "Retirer la photo",
      cropTitle: "Recadre ta photo",
      cropConfirm: "Valider",
      cropCancel: "Annuler",
      zoomLabel: "Zoom",
      errorNotImage: "Ce fichier n'est pas une image.",
      errorTooLarge: "Image trop lourde (5 Mo maximum).",
      errorUpload: "Échec de l'envoi de la photo. Réessaie.",
    },
    colors: {
      red: "Casque rouge",
      orange: "Casque orange",
      amber: "Casque jaune",
      green: "Casque vert",
      teal: "Casque turquoise",
      cyan: "Casque cyan",
      blue: "Casque bleu",
      indigo: "Casque indigo",
      purple: "Casque violet",
      pink: "Casque rose",
      white: "Casque blanc",
      slate: "Casque gris",
    },
  },
  theme: {
    pageTitle: "Thème",
    subtitle: "Personnalise les couleurs de l'application.",
    applied: "Appliqué",
    themes: {
      boxbox:   "BoxBox",
      ferrari:  "Ferrari",
      mercedes: "Mercedes",
      mclaren:  "McLaren",
      redbull:  "Red Bull",
      aston:    "Aston Martin",
    },
  },
  profile: {
    pageTitle: "Mon profil",
    statsLeague: "ligue",
    statsLeaguePlural: "ligues",
    statsPts: "pts cette saison",
    editPseudo: "Modifier le pseudo",
    editAvatar: "Changer d'avatar",
    theme: "Thème",
    notifications: "Notifications",
    whatsNew: "Nouveautés",
    accessibility: "Mode accessibilité",
    fontSize: "Taille du texte",
    fontSizeNormal: "Normal",
    fontSizeLarge: "Grand",
    fontSizeXlarge: "Très grand",
    splashSound: "Son au lancement",
    signOut: "Se déconnecter",
    back: "Profil",
    // Sous-page edit pseudo
    editPseudoTitle: "Modifier le pseudo",
    pseudoLabel: "Pseudo",
    save: "Enregistrer",
    saving: "Enregistrement…",
    savedOk: "Profil mis à jour !",
    // Sous-page edit avatar
    editAvatarTitle: "Changer d'avatar",
    // Zone dangereuse
    dangerZone: "Zone dangereuse",
    deleteAccount: "Supprimer mon compte",
    deleteWarning: "Ton compte sera anonymisé — tes scores passés sont conservés pour l'intégrité du classement. Cette action est irréversible.",
    deleteConfirm: "Confirmer",
    deleteCancel: "Annuler",
    deleting: "Suppression…",
  },
  install: {
    bannerTitle:      "Installer BoxBox",
    bannerSubtitle:   "Accès rapide depuis ton écran d'accueil.",
    ctaAndroid:       "Installer",
    close:            "Fermer",
    iosInstructions:  "Appuie sur",
    iosThen:          "puis « Sur l'écran d'accueil »",
    browserInstructions: "Depuis le menu de ton navigateur, choisis « Installer l'application ».",
    profileRow:       "Installer l'app",
  },
  notifications: {
    title: "Notifications",
    subtitle: "Toutes activées par défaut · via Web Push (iOS 16.4+ & Android).",
    deadline: "Deadline pronos & items",
    deadlineSub: "1h avant chaque session",
    scores: "Résultats & scores",
    scoresSub: "Provisoires, définitifs, classement",
    items: "Items reçus",
    itemsSub: "Révélation après la course",
    gpApproach: "GP approche",
    gpApproachSub: "J-2 avant le week-end",
    imminence: "Session imminente",
    imminenceSub: "10 min avant le début",
    imminenceScopeAll: "Toutes les sessions",
    imminenceScopeStakes: "Sessions à enjeu",
    imminenceScopeNone: "Aucune",
    imminenceScopeSaving: "Enregistrement…",
    imminenceScopeError: "Échec de l'enregistrement, réessaie.",
    announcements: "Nouveautés & annonces",
    announcementsSub: "Nouvelles features et mises à jour",
    back: "Profil",
    unsupported: "Notifications push non supportées sur cet appareil.",
    denied: "Notifications bloquées dans les réglages de ton navigateur.",
  },
  whatsNew: {
    title: "Nouveautés",
    back: "Profil",
    empty: "Rien de neuf pour l'instant — reviens bientôt !",
  },
  // Erreurs des Server Actions — les actions retournent des codes (lib/actions/errors.ts),
  // le client traduit via translateActionError. Copy reprise à l'identique des anciennes
  // chaînes en dur (#181). Le test d'exhaustivité garantit une clé par code.
  actionErrors: {
    notAuthenticated: "Non authentifié",
    invalidRequest: "Requête invalide",
    unexpected: "Erreur inattendue",
    serverError: "Erreur serveur",
    itemOnlyCurrentGp: "Les items ne sont jouables que sur le GP en cours",
    noSessionsForGp: "Aucune session trouvée pour ce GP",
    unsupportedItemType: "Type d'item non supporté",
    itemDeadlinePassed: "Deadline passée — cet item est verrouillé pour ce GP",
    sessionUnavailable: "Session indisponible pour ce GP",
    sessionAlreadyStarted: "Session déjà démarrée — trop tard pour la cibler",
    notLeagueMember: "Tu n'es pas membre de cette ligue",
    itemAlreadyPlayedThisWeekend: "Tu as déjà joué un item ce week-end dans cette ligue",
    itemExhausted: "Item épuisé pour cette saison",
    cannotTargetSelf: "Tu ne peux pas te cibler toi-même",
    targetNotLeagueMember: "La cible n'est pas membre de cette ligue",
    targetRequired: "Cible requise",
    driverRequired: "Pilote requis",
    constructorRequired: "Écurie requise",
    constructorUnknown: "Écurie inconnue pour cette saison",
    invalidSession: "Session invalide",
    adminOnly: "Action réservée à l'administrateur de la ligue",
    updateFailed: "Erreur lors de la mise à jour",
    cannotTransferToSelf: "Transfert vers soi-même impossible",
    transferFailed: "Erreur lors du transfert",
    memberNotFound: "Membre introuvable",
    regenerateFailed: "Erreur lors de la régénération",
    inviteCodeGenerationFailed: "Impossible de générer un code unique",
    sessionNotFound: "Session introuvable",
    sessionLocked: "Session verrouillée",
    tooManyDrivers: "Trop de pilotes sélectionnés",
    duplicateDriver: "Un pilote apparaît en double",
    unknownDriver: "Sélection invalide (pilote inconnu)",
    fastestLapRaceOnly: "Meilleur tour réservé à la course",
    seasonPredictionsLocked: "Deadline passée — pronostics saison verrouillés",
    seasonPredictionsNotLockedYet: "Les pronostics saison ne sont pas encore verrouillés",
    seasonItemsLocked: "Deadline passée — items saison verrouillés",
    seasonPredictionRequiredFirst: "Soumets d'abord ton pronostic saison avant d'utiliser cet item",
    entriesCountRequired: "{count} entrées requises",
    invalidCode: "Code invalide : {code}",
    duplicateEntries: "Doublons détectés",
    invalidPositions: "Positions invalides",
    pseudoLength: "Le pseudo doit faire entre 3 et 20 caractères",
    pseudoChars: "Lettres, chiffres et underscore uniquement",
    pseudoTaken: "Ce pseudo est déjà pris.",
    helmetRequired: "Choisis un casque pour continuer.",
    deleteAccountFailed: "Erreur lors de la suppression du compte",
    somethingWentWrong: "Une erreur est survenue. Réessaie.",
    leagueNameInvalid: "Nom invalide (2–50 caractères)",
    leagueSizeInvalid: "Taille invalide (2–20 joueurs)",
    inviteCodeRequired: "Code d'invitation requis",
    leagueNotFound: "Ligue introuvable — vérifie le code",
    leagueClosed: "Les inscriptions sont fermées pour cette ligue",
    alreadyLeagueMember: "Tu es déjà membre de cette ligue",
    leagueFull: "Cette ligue a atteint son nombre maximum de membres",
  },
} as const;
